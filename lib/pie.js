...
exports.createSvg = function(sliceValues, chartOptions, callback) {
	...
	*/
	...
	function getSvgRect() {
		return (chartOptions.bgColor && chartOptions.bgColor !== 'none')
	...
	function getSvgPaths() {
		...
		if (INSTANCE.slices.length === 1) {
			let pathAttribs = getPathAttribs(0);
			shapes[0] = util.format('<circle %s cx="%s" cy="%s" r="%s" />', pathAttribs, radius, radius, pieWidth);
		}
		else {
