...
exports.createSvg = function(sliceValues, chartOptions, callback) {
	...
	*/
	...
	function getSvgStyle() {
		return util.format('<svg height="%d" width="%d" version="1.1" xmlns="http://www.w3.org/2000/svg">', chartOptions.size, chartOptions.size);
	}


	function getSvgRect() {
		return util.format('<rect x="0" y="0" width="%d" height="%d" fill="%s" stroke="none"></rect>', chartOptions.size, chartOptions.size, chartOptions.bgColor);
	}
